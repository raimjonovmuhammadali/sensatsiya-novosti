...
const updateActive = async (item) => {
  ...
  const updatedData = {
    name: item.name || '',
    advertisement_type: item.advertisement_type || 'sidebar',
    description: item.description || '',
    video: item.video || '',
    link: item.link || '',
    active_time: item.active_time || new Date().toISOString(), // fallback
    active: !newActive,
  };
...
const createNewAd = async () => {
  ...
  formData.append("advertisement_type", newItem.value.advertisement_type);
  formData.append("description", newItem.value.description);
  formData.append("link", newItem.value.link);
  formData.append("active_time", newItem.value.active_time);
  formData.append("active", newItem.value.active);
...
const deleteAd = async (id) => {
  if (confirm("Haqiqatan ham o‘chirmoqchimisiz?")) {
    ...
    if (!error.value) {
      addData.value = addData.value.filter((ad) => ad.id !== id);
    } else {
      ...
      >
        <thead>
          <tr class="bg-gray-700 text-gray-300 text-sm sm:text-base">
            ...
            <th class="p-3 sm:p-4 hidden lg:table-cell">Turi</th>
            <th class="p-3 sm:p-4 hidden md:table-cell">Tavsif</th>
            <th class="p-3 sm:p-4">Holat</th>
            ...
            <td class="p-3 sm:p-4 text-center">
              <label class="inline-flex items-center cursor-pointer">
                <input
                  ...
                  class="sr-only"
                />
                <span class="relative w-10 h-6 bg-gray-600 rounded-full">
                  <span
                    :class="
                      item.active
                    ...
                    "
                    class="absolute left-1 top-1 w-4 h-4 rounded-full transition-transform"
                  ></span>
                </span>
              </label>
            </td>
    ...
    >
      ...
      >
        ...
        <form @submit.prevent="saveChanges" class="space-y-4">
          ...
          <div>
            <label
              for="edit-advertisement_type"
              class="block text-sm font-medium text-gray-300"
              >Turi</label
            >
            <select
              v-model="editedItem.advertisement_type"
              id="edit-advertisement_type"
              required
              class="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-teal-400"
            >
          ...
          <div>
            <label class="flex items-center space-x-2">
              <input
                ...
                class="form-checkbox h-5 w-5 text-teal-400 bg-gray-700 border-gray-600 rounded focus:ring-teal-400"
              />
              <span class="text-sm text-gray-300">Faol</span>
            </label>
          </div>
    ...
    >
      ...
      >
        ...
        <form @submit.prevent="createNewAd" class="space-y-4">
          ...
          <div>
            <label
              for="new-advertisement_type"
              class="block text-sm font-medium text-gray-300"
              >Turi</label
            >
            <select
              v-model="newItem.advertisement_type"
              id="new-advertisement_type"
              required
              class="mt-1 block w-full px-3 py-2 bg-gray-700 border border-gray-600 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-teal-400"
            >
