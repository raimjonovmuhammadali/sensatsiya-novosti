  ...
  <main class="max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-6 sm:py-10">
    <div class="mb-8 sm:mb-10">
      <h1 class="text-xs sm:text-sm font-extrabold text-gray-900 mb-4 flex items-center">
        <span v-if="isLoadingCategories" class="w-32 h-4 bg-gray-200 rounded animate-pulse"></span>
        <span v-else>{{ getCategoryTitle(newsData?.category) }}</span>
      </h1>
      <p class="text-lg sm:text-xl text-gray-600">
        <span v-if="isLoadingNews" class="w-2/3 h-6 bg-gray-200 rounded animate-pulse block"></span>
    ...
    <div class="grid lg:grid-cols-4 gap-6 sm:gap-10">
      <div class="lg:col-span-3 flex flex-col gap-8 sm:gap-10">
        ...
        <section class="w-full">
          <div class="w-full bg-white rounded-xl shadow-lg overflow-hidden transition-shadow duration-300">
            <div v-if="isLoadingNews" class="h-72 bg-gray-200 animate-pulse"></div>
            <template v-else>
              ...
              <template v-else>
                <div class="h-72 bg-gray-100 flex items-center justify-center text-gray-400 text-sm">
                  Media mavjud emas
                </div>
              </template>
            </template>
            ...
            <div class="p-4 sm:p-6">
              <div v-if="isLoadingNews">
                <div class="w-20 h-5 bg-red-200 rounded mb-4 animate-pulse"></div>
                <div class="w-full h-6 bg-gray-200 rounded mb-3 animate-pulse"></div>
                <div class="w-full h-24 bg-gray-100 rounded animate-pulse"></div>
              </div>
              <template v-else>
                <span class="bg-red-700 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold mb-3 inline-block">
                ...
                <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs sm:text-sm text-gray-600 gap-4">
              ...
              </svg>{{ newsData?.views }}</span>
                    <span class="hidden sm:inline">•</span>
                    <span>{{ new Date(newsData?.created_date).toLocaleString() }}</span>
                    <span class="hidden sm:inline">•</span>
                    <span>{{ calculateReadTime(newsData?.text) }} o'qiladi</span>
                  </div>
                  <div class="flex items-center space-x-2">
                    <button class="px-2 sm:px-3 py-1 bg-blue-100 text-blue-600 rounded-full text-xs sm:text-sm hover:bg-blue-200 transition-all">
                      <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M8.684 13.342A3 3 0 119.5 12a3 3 0 11-.816 1.342zm0 0l6.632 3.316m-6.632-6l6.632-3.316"></path>
                      </svg>
                      Ulashish
                    </button>
                    <button class="px-2 sm:px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs sm:text-sm hover:bg-gray-200 transition-all">
                      <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <div class="w-full flex flex-wrap gap-6">
            ...
            >
              <div class="bg-white rounded-xl shadow-lg hover:shadow-xl overflow-hidden transition-shadow duration-300">
                <img v-if="article.image" :src="article.image" :alt="article.title" class="w-full h-36 object-cover" />
                <div class="p-4 sm:p-6">
...
const getCategoryTitle = (categoryId) => {
  return (
    categoiresData.value?.find((cat) => cat.id === categoryId)?.title || "Nomaʼlum"
  );
};
